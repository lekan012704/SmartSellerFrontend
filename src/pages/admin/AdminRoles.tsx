...
const Roles = () => {
  ...
  // Create form — includes permissions
  ...
  // Edit form — name, description, active only (no permissions)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Role created" });
      setCreateOpen(false);
      setCreateForm({ roleName: "", description: "", isActive: true, claims: [] });
      loadAll();
    } catch (err) {
      const msg = err instanceof ApiError ? err.message : "Failed";
      toast({ title: "Couldn't create role", description: msg, variant: "destructive" });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await RoleAPI.update({
        roleId: editing.roleId || editing.id || "",
        ...editForm,
        claims: editing.claims || [], // preserve existing claims unchanged
      });
      toast({ title: "Role updated" });
      setEditOpen(false);
      setEditing(null);
  ...
  return (
    <AdminLayout>
                  ...
                  <Card key={r.roleId || r.id || r.roleName} className="p-4 hover:shadow-glow transition-shadow">
                    ...
                    {r.claims && r.claims.length > 0 && (
                      <p className="text-xs text-muted-foreground mb-3">
                        {r.claims.length} permission{r.claims.length !== 1 ? "s" : ""}
                      </p>
                    )}
                    <div className="flex gap-1 flex-wrap">
                      <Button size="sm" variant="ghost" onClick={() => openUsersInRole(r)}>
      ...
      {/* Create role — includes PermPicker */}
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Create role</DialogTitle>
            <DialogDescription>Give the role a name and assign permissions.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-3">
            <Input
            ...
            <PermPicker
              all={allPerms}
              selected={createForm.claims}
              onToggle={(p) => togglePerm(p, createForm.claims, (l) => setCreateForm({ ...createForm, claims: l }))}
            />
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => setCreateOpen(false)}>Cancel</Button>
              <Button type="submit" disabled={submitting} className="gradient-primary text-primary-foreground border-0">
      ...
      {/* Edit role — name, description, active only. No permissions picker */}
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit role</DialogTitle>
            <DialogDescription>Update the role name or description. To change permissions, use the Grant user permissions flow.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleUpdate} className="space-y-3">
            <Input
              placeholder="Role name"
            ...
            {users.length > 0 ? (
              <div>
                ...
                >
                  <option value="">Select user...</option>
                  {users.map((u) => (
                    <option key={u.id || u.userId || u.email} value={u.id}>
